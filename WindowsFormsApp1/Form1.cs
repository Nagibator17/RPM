...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        public bool ChekIfMapsNotEmpty()
        {
            bool isEmpty1 = true;
            bool isEmpty2 = true;
        ...
        public void PlayerShoot(object sender, EventArgs e)
        {
           ...
           bool playerTurn  = Shoot(enemyMap, pressedButton);
            ...
            if(!ChekIfMapsNotEmpty())
            {
                this.Controls.Clear();
                Init();
            }
        }
        public bool Shoot(int [,] map, Button presseButton)
        {
            bool hit = false;
            if (isPlaying)
            {
                int delta = 0;
                if (presseButton.Location.X > 320)
                    delta = 320;

                if (map[presseButton.Location.Y / cellSize, (presseButton.Location.X - delta) / cellSize] != 0)
                {
                    hit = true;
                    map[presseButton.Location.Y / cellSize, (presseButton.Location.X - delta) / cellSize] = 0;
                    presseButton.BackColor = Color.Blue;
                    presseButton.Text = "X";
                }
                else
                {
                    ...
                    presseButton.BackColor = Color.Black;
                    presseButton.Text = "X";
                }
